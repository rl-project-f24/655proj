...
if __name__ == "__main__":
    ...
    for d in range(args.D):
        ...
        segments = {k: v[0] for k, v in trajectories.items()}
        ...
        all_predicted_rewards.append(np.mean(returns_predicted_eval))
        all_actual_rewards.append(np.mean(returns_actual_eval))
        all_random_rewards.append(np.mean(returns_random_eval))
        iterations.append(d + 1)  # Track iteration count for the x-axis
    ...
    plt.figure()
    plt.plot(iterations, all_predicted_rewards, label='Predicted Rewards', marker='o')
    plt.plot(iterations, all_actual_rewards, label='Actual Rewards', marker='o')
    plt.plot(iterations, all_random_rewards, label='Random Policy', marker='o')
    plt.xlabel('Outer Iteration (d)')
    plt.ylabel('Average Episodic Return')
    plt.title('Performance Comparison Over Outer Iterations')
    plt.legend()
    plt.savefig(f"runs/{run_name}/performance_comparison_over_iterations.png")
    plt.show()
