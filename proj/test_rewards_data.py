...
def evaluate_random_policy(env_fn, num_episodes=10):
    returns = []
    for _ in range(num_episodes):
        env = env_fn()
        obs, _ = env.reset()
...
if __name__ == "__main__":
    ...
    num_trajectories_list = [40, 100, 160]

    # Lists to store results for each num_trajectories setting
    results_predicted = []
    results_actual = []
    results_random = []
    for num_trajectories in num_trajectories_list:
        reward_predictor = RewardPredictorNetwork(envs.single_observation_space).to(device)
        reward_trainer = RewardTrainer(reward_predictor, device=device, lr=args.reward_learning_rate, corruption_percentage=args.corruption_percentage)
        agent = Agent(envs).to(device)
        for d in range(args.D):
            ...
            trajectories = collector.collect_trajectories(num_trajectories)
        ...
        # Evaluate both agents and the random policy at d-1
        ...
        returns_random_eval = evaluate_random_policy(env_fn, num_episodes=10)

        # Store the final mean rewards for each setting
        results_predicted.append(np.mean(returns_predicted_eval))
        results_actual.append(np.mean(returns_actual_eval))
        results_random.append(np.mean(returns_random_eval))
    ...
    plt.figure()
    plt.bar(['Predicted Rewards', 'Actual Rewards', 'Random Policy'], 
            [np.mean(returns_predicted_eval), np.mean(returns_actual_eval), np.mean(returns_random_eval)],
            yerr=[np.std(returns_predicted_eval), np.std(returns_actual_eval), np.std(returns_random_eval)])
    ...
    plt.figure()
    plt.plot(num_trajectories_list, results_predicted, label='Predicted Rewards', marker='o')
    plt.plot(num_trajectories_list, results_actual, label='Actual Rewards', marker='o')
    plt.plot(num_trajectories_list, results_random, label='Random Policy', marker='o')
    plt.xlabel('Number of Trajectories')
    plt.ylabel('Average Episodic Return at d-1')
    plt.title('Performance Comparison with Different Numbers of Trajectories')
    plt.legend()
    plt.savefig(f"runs/{run_name}/performance_comparison_num_trajectories.png")
    plt.show()
